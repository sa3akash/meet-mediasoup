...
export function MeetingRoomClient({ slug, initialMeeting, currentUser }: MeetingRoomClientProps) {
  ...
  const soup = useMediasoup(
    slug, state.displayName, currentUser?.id,
    {
      ...
      onReactionReceived: (emoji: string) => { state.setActiveReaction(emoji); setTimeout(() => state.setActiveReaction(null), 2500); },
      ...
      onPollNew: (p: any) => state.setPolls((prev) => [p, ...prev.filter((x) => x.id !== p.id)]),
      onPollUpdated: (p: any) => state.setPolls((prev) => prev.map((x) => (x.id === p.id ? p : x))),
  ...
  return (
    <div className="relative w-screen h-screen bg-neutral-950 flex flex-col overflow-hidden select-none">
      <div className="flex-1 flex w-full h-full overflow-hidden">
        ...
        <MeetingSidebars
          ...
          onSendMessage={handleSendMessage} onReactMessage={soup.reactToChatMessage} onDeleteMessage={soup.deleteChatMessage}
          ...
          onCreatePoll={soup.createPoll} onVotePoll={soup.votePoll} onEndPoll={soup.endPoll}
      ...
      <ControlBar onLeave={handleLeave} onOpenHostControls={() => state.setIsHostControlsOpen(true)} onOpenScreenShareModal={() => state.setIsScreenShareModalOpen(true)} onOpenRecordingModal={() => state.setIsRecordingModalOpen(true)} />
