...
interface MeetingListItemCardProps {
  meeting: MeetingItem;
  isCopied: boolean;
...
export function MeetingListItemCard({
  meeting: m,
  isCopied,
  onCopy,
  onCancel,
}: MeetingListItemCardProps) {
  const isRecurring = m.type === "RECURRING";
  const isInstant = m.type === "INSTANT";

  return (
    <div className="p-5 rounded-3xl bg-neutral-900/80 border border-white/10 hover:border-white/20 transition-all flex flex-col justify-between gap-4 group">
